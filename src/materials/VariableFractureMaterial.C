...
#include "VariableFractureMaterial.h"
...
FractureMaterial::validParams()
{
  ...
  params.addRequiredParam<MaterialPropertyName>(
      "Gc", "fracture energy release rate, alert: must be a material property");
  ...
  params.addParam<MaterialPropertyName>(
      "mob_name", "mobility", "name of the material that holds the mobility");

  return params;
}

VariableFractureMaterial::VariableFractureMaterial(const InputParameters & parameters)
  : Material(parameters),
    _Gc(getMaterialPropertyByName<Real>(getParam<MaterialPropertyName>("Gc"))),
    _L(getFunction("L")),
...
void
VariableFractureMaterial::initQpStatefulProperties()
{
...
void
VariableFractureMaterial::computeQpProperties()
{
  Real L = _L.value(0.0, _q_point[_qp]);
  Real c0 = _w_norm.value(0.0, _q_point[_qp]);
