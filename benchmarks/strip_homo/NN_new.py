...
data = pd.read_csv('./def2/stress_xx_sample_def_Data_0000.csv')
...
for i in range(num_step):
    if (i+1) < 10:
        data = pd.read_csv('./def2/' + file_name + '0' + str(i+1) + '.csv')
    else:
        data = pd.read_csv('./def2/' + file_name + str(i+1) + '.csv')
...
# y = y[1:100]
model.fit(x.reshape(-1,8), y, epochs=100, verbose=1)
